...
================================================================
  ...
  Parametros opcionais:
     ...
     -SkipMirror      nao prepara nem confere o aparelho para espelhamento (scrcpy)

  APOS FORMATAR O CELULAR: a formatacao APAGA as Opcoes do desenvolvedor, a
  Depuracao USB e a autorizacao deste PC. Refaca isso NA MAO no aparelho antes
  de rodar este script -- nada aqui funciona sem adb, nem o diagnostico do painel.
================================================================
#>
param(
  [switch]$SkipDebloat,
  ...
  [switch]$SkipMirror,            # preparar/conferir o aparelho para espelhamento (scrcpy)
...
# Alguns ROMs (realme/ColorOS, MIUI) NAO imprimem 'topResumedActivity', so 'mResumedActivity'.
# Olhando so p/ um dos dois a deteccao falha e o Setup-Island fica girando ate desistir
# mesmo com o perfil de trabalho ja criado. Por isso aceitamos os dois.
function Get-Foreground { return ((& $Adb shell dumpsys activity activities | Select-String "topResumedActivity|mResumedActivity" | Select-Object -First 1) -join ' ') }
...
# ================= 8c. PREPARAR PARA ESPELHAMENTO (scrcpy) =================
# Deixa o aparelho pronto para o espelhamento com teclado e toque virtuais (UHID).
# Duas coisas acontecem aqui, e elas sao diferentes:
#  - CONFIGURA o que da para configurar: "permanecer ativo" de forma PERMANENTE.
#    Nao confundir com o "svc power stayon" da linha 120: aquele e temporario, so
#    para o aparelho nao dormir durante esta automacao. Os dois escrevem na MESMA
#    chave (stay_on_while_plugged_in), e por isso a reversao no resumo final so
#    zera a chave quando ESTA etapa foi pulada -- senao ela apagaria o ajuste.
#  - CONFERE o que nao se configura: /dev/uhid, grupo uhid, uiautomator e a versao
#    do Android. Se algo faltar, o espelhamento ate abre, mas o teclado e o toque
#    nao funcionam -- e melhor descobrir agora do que na hora do uso.
# As animacoes NAO sao mexidas aqui de proposito: a etapa 8a ja zera as tres. Esta
# etapa so confere e avisa caso 8a tenha sido pulada, porque o overlay do esqueleto
# falha enquanto a tela anima.
if(-not $SkipMirror){
  Write-Host "`n== Preparar para espelhamento ==" -ForegroundColor Cyan
  if($DryRun){
    Write-Host "  [seria feito] manter a tela ativa enquanto carregando" -ForegroundColor Yellow
    Write-Host "  [seria conferido] /dev/uhid, grupo uhid, uiautomator, versao do Android" -ForegroundColor Yellow
  } else {
    # 7 = tomada + USB + sem fio (soma dos tres tipos de carregamento)
    & $Adb shell settings put global stay_on_while_plugged_in 7 2>$null | Out-Null
    $rStay = ((& $Adb shell settings get global stay_on_while_plugged_in) -join '').Trim()
    if($rStay -eq '7'){ Write-Host "  Manter tela ativa ao carregar: [ativado]" -ForegroundColor Green }
    else { Write-Host ("  Manter tela ativa: [nao confirmado] (valor={0})" -f $rStay) -ForegroundColor Yellow }

    # --- pre-requisitos do modo UHID (nao sao configuraveis: ou a ROM tem, ou nao) ---
    $sdk = ((& $Adb shell getprop ro.build.version.sdk) -join '').Trim()
    $rel = ((& $Adb shell getprop ro.build.version.release) -join '').Trim()
    if(($sdk -as [int]) -ge 30){
      Write-Host ("  Android {0} (API {1}): [ok]" -f $rel, $sdk) -ForegroundColor Green
    } else {
      Write-Host ("  Android {0} (API {1}): [antigo] abaixo do 11 o shell costuma nao acessar /dev/uhid" -f $rel, $sdk) -ForegroundColor Yellow
    }

    $uhidDev = ((& $Adb shell "ls /dev/uhid 2>&1") -join '').Trim()
    $uhidGrp = ((& $Adb shell id) -join '')
    if(($uhidDev -notmatch 'No such file') -and ($uhidGrp -match 'uhid')){
      Write-Host "  Teclado/toque virtuais (/dev/uhid): [ok]" -ForegroundColor Green
    } else {
      Write-Host "  Teclado/toque virtuais (/dev/uhid): [INDISPONIVEL]" -ForegroundColor Red
      Write-Host "    -> a ROM nao libera o /dev/uhid para o adb; sem isso o espelho" -ForegroundColor Red
      Write-Host "       abre mas nao digita nem clica. Nao ha configuracao que resolva." -ForegroundColor Red
    }

    $ua = ((& $Adb shell which uiautomator) -join '').Trim()
    if($ua){ Write-Host "  uiautomator (overlay de telas protegidas): [ok]" -ForegroundColor Green }
    else { Write-Host "  uiautomator: [ausente] o espelho funciona, mas sem o overlay" -ForegroundColor Yellow }

    # Confere as animacoes em vez de reescrever: quem zera e a etapa 8a.
    $anim = ((& $Adb shell settings get global window_animation_scale) -join '').Trim()
    if($anim -eq '0' -or $anim -eq '0.0'){
      Write-Host "  Animacoes desligadas: [ok]" -ForegroundColor Green
    } else {
      Write-Host ("  Animacoes em {0}: [atencao] rode a etapa 'Deixar mais rapido'" -f $anim) -ForegroundColor Yellow
      Write-Host "    -> o overlay de telas protegidas falha enquanto a tela anima." -ForegroundColor Yellow
    }
  }
}
...
if(-not $DryRun){
  ...
  # Reverte o stayon temporario da linha 120. ATENCAO: o "svc power stayon" e a
  # etapa 8c escrevem na MESMA chave (stay_on_while_plugged_in), entao reverter
  # cego aqui apagaria o que a etapa acabou de configurar.
  if($SkipMirror){ & $Adb shell svc power stayon false 2>$null | Out-Null }
  else           { & $Adb shell settings put global stay_on_while_plugged_in 7 2>$null | Out-Null }
}
